...
async function actualizarDatos() {
  try {
    ...
    // Actualizar gauges con tu función ya existente
    createOrUpdateGauge('tempGauge', parseFloat(data.temperatura), 50);
    createOrUpdateGauge('humGauge', parseFloat(data.humedad), 100);
    createOrUpdateGauge('presGauge', parseFloat(data.presion), 1100);
    createOrUpdateGauge('airGauge', parseFloat(data.calidad_Aire), 500);

    // Verificar alarmas
    verificarAlarma('temp', data.temperatura);
    verificarAlarma('hum', data.humedad);
    verificarAlarma('pres', data.presion);
    verificarAlarma('air', data.calidad_Aire);
